...
import type { InsurancePage } from '../siteData';
import { 
    CheckCircleIcon, UsersIcon, CogIcon, ClockIcon, CurrencyIcon, TreasuryIcon, 
    ReportsIcon, ChartBarIcon, DocumentDuplicateIcon, FinanceIcon,
    IndustryIcon, StoreIcon, BuildingOfficeIcon, CalculatorIcon, LinkIcon, ShieldCheckIcon, SmsPanelIcon, PayrollIcon
} from './Icons';
...
const InsurancePage: React.FC<InsurancePageProps> = ({ content }) => {
  const { hero, importance, features, integrations, advancedFeatures, targetIndustries, finalCta } = content;
  ...
  const tabs = [
    ...
    { id: 'targetIndustries', title: 'مخاطبین اصلی' },
  ];
    ...
    <div className="bg-[var(--color-background)] text-[var(--color-text-primary)] overflow-x-hidden">
      ...
      <section id="features" className="py-24 bg-[var(--color-background)]">
        <div className="container mx-auto px-6">
            ...
            {activeTab === 'targetIndustries' && (
                 <div className="animate-tab-content-fade-in grid grid-cols-2 md:grid-cols-4 gap-8">
                    {(targetIndustries.items || []).map((item, index) => {
                         return (
                             <Animated key={index} delay={100 + index * 100}>
                                <div className="group bg-white p-6 rounded-lg shadow-lg flex flex-col items-center justify-center gap-4 h-full text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-primary/20">
                                   {renderIcon(item.icon, item.name, "h-10 w-10 text-[var(--color-primary)] transition-transform duration-300 group-hover:scale-110")}
                                   <h3 className="text-base font-semibold text-[var(--color-text-secondary)]">{item.name}</h3>
                                </div>
                            </Animated>
                        );
                    })}
