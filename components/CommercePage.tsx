...
import type { CommercePage as CommercePageType } from '../siteData';
import { 
    ...
    FinanceIcon, SalesIcon, CurrencyIcon, TreasuryIcon, IndustryIcon
} from './Icons';
...
const iconMap: { [key: string]: React.ElementType } = {
  ...
  ClockIcon, ChartBarIcon, CogIcon, CalculatorIcon, BuildingOfficeIcon
};

interface CommercePageProps {
  content: CommercePageType;
...
const CommercePage: React.FC<CommercePageProps> = ({ content, setView }) => {
          ...
          <Animated delay={200} className="flex flex-col gap-6">
            <nav aria-label="Breadcrumb">
              <ol className="flex items-center gap-1 text-sm text-[var(--color-text-secondary)]">
                ...
                <li><span className="opacity-50">/</span></li>
                <li className="font-medium text-[var(--color-text-primary)]" aria-current="page">نرم‌افزار بازرگانی</li>
              </ol>
            </nav>
            ...
            <div className="text-xs text-[var(--color-text-secondary)]">
                <p>کسب‌وکارهای هدف: 
                    {(keyAspects.items.find(item => item.title === 'کسب‌وکارهای هدف')?.points || []).slice(0, 3).map((target, i) => (
