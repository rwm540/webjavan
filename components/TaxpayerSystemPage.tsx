...
import type { TaxpayerSystemPage as TaxpayerSystemPageType } from '../siteData';
import { 
    UsersIcon, CogIcon, ReportsIcon, UploadCloudIcon, DatabaseIcon, ChartBarIcon, 
    CheckCircleIcon, LinkIcon, ShieldCheckIcon, SmsPanelIcon
} from './Icons';

const iconMap: { [key: string]: React.ElementType } = {
  UsersIcon, CogIcon, ReportsIcon, UploadCloudIcon, DatabaseIcon, ChartBarIcon, CheckCircleIcon, LinkIcon, ShieldCheckIcon, SmsPanelIcon
};

interface TaxpayerSystemPageProps {
  content: TaxpayerSystemPageType;
...
const TaxpayerSystemPage: React.FC<TaxpayerSystemPageProps> = ({ content, setView }) => {
          ...
          <Animated delay={200} className="flex flex-col gap-6">
            <nav aria-label="Breadcrumb">
              <ol className="flex items-center gap-1 text-sm text-[var(--color-text-secondary)]">
                ...
                <li><span className="opacity-50">/</span></li>
                <li className="font-medium text-[var(--color-text-primary)]" aria-current="page">سامانه مودیان</li>
              </ol>
            </nav>
            ...
            {activeTab === 'keyAspects' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    {(keyAspects.items || []).map((spec, index) => (
                         <div key={index} className="bg-white p-8 rounded-2xl shadow-xl border animate-fade-in-up">
                            <div className="flex items-center gap-4 mb-8">
                                {renderIcon(spec.icon, spec.title, "w-12 h-12 text-[var(--color-primary)]")}
...
export default TaxpayerSystemPage;