...
import type { IndustrialPage as IndustrialPageType } from '../siteData';
import { 
    ...
    FinanceIcon, SalesIcon, CurrencyIcon, TreasuryIcon, IndustryIcon, ProductionIcon, PayrollIcon, TrashIcon
} from './Icons';
...
const iconMap: { [key: string]: React.ElementType } = {
  ...
  AccountingIcon, CustomerClubIcon, UserManagementIcon, DashboardIcon, BarcodeScannerIcon,
  ReceiptPrinterIcon, CreditCardTerminalIcon, SmsPanelIcon, ECommerceIcon, CheckCircleIcon,
  DesktopComputerIcon, StoreIcon, ShieldCheckIcon, FinanceIcon, SalesIcon, CurrencyIcon, TreasuryIcon, IndustryIcon,
  ClockIcon, ChartBarIcon, CogIcon, CalculatorIcon, BuildingOfficeIcon, ProductionIcon, PayrollIcon, TrashIcon
};

interface IndustrialPageProps {
  content: IndustrialPageType;
...
const IndustrialPage: React.FC<IndustrialPageProps> = ({ content, setView }) => {
  const { hero, challenges, modules, keyAspects, finalCta } = content;
  ...
  useEffect(() => {
    if (modules.items && modules.items.length > 0 && !activeFeature) {
      setActiveFeature(modules.items[0]);
    }
  }, [modules.items, activeFeature]);

  const tabs = [
    { id: 'modules', title: 'امکانات و ماژول‌ها' },
          ...
          <Animated delay={200} className="flex flex-col gap-6">
            <nav aria-label="Breadcrumb">
              <ol className="flex items-center gap-1 text-sm text-[var(--color-text-secondary)]">
                ...
                <li><span className="opacity-50">/</span></li>
                <li className="font-medium text-[var(--color-text-primary)]" aria-current="page">نرم‌افزار صنعتی</li>
              </ol>
            </nav>
            ...
            <div className="text-xs text-[var(--color-text-secondary)]">
                <p>صنایع هدف: 
                    {(keyAspects.items.find(item => item.title.includes('صنایع'))?.points || []).slice(0, 3).map((target, i) => (
                        <span key={i} className="bg-gray-200/50 border border-gray-300/50 text-gray-600 px-2 py-0.5 rounded-md mr-1">{target}</span>
                    ))}
                </p>
            </div>
          </Animated>
            ...
            {activeTab === 'modules' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {(modules.items || []).map((feature, index) => {
