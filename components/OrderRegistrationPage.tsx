...
import type { OrderRegistrationPage as OrderRegistrationPageType } from '../siteData';
import { 
    CheckCircleIcon, DocumentDuplicateIcon, WarehouseIcon, ReportsIcon, CurrencyIcon, LinkIcon,
    TreasuryIcon, CrmIcon, SalesIcon, FinanceIcon, WarehouseManagementIcon
} from './Icons';

const iconMap: { [key: string]: React.ElementType } = {
  DocumentDuplicateIcon, WarehouseIcon, ReportsIcon, CurrencyIcon, LinkIcon,
  TreasuryIcon, CrmIcon, SalesIcon, FinanceIcon, WarehouseManagementIcon
};
...
const OrderRegistrationPage: React.FC<{ content: OrderRegistrationPageType }> = ({ content }) => {
  const { hero, introduction, keyObjectives, features, integrations, reports, benefits, summary, finalCta } = content;
  ...
  return (
    <div className="bg-[var(--color-background)] text-[var(--color-text-primary)] overflow-x-hidden">
      ...
      {/* Introduction & Key Objectives Section */}
      <section className="py-24 bg-[var(--color-card-background)]">
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
             <div className="lg:col-span-1">
                <Animated>
                    <h2 className="text-2xl md:text-3xl font-bold mb-6">مقدمه</h2>
                    <p className="text-base md:text-lg text-[var(--color-text-secondary)] leading-relaxed">{introduction}</p>
                </Animated>
            </div>
            <div className="lg:col-span-1">
                 <Animated delay={200}>
                    <div className="bg-gray-50 p-8 rounded-xl border border-[var(--color-border)]">
                        <h3 className="text-xl font-bold mb-6 text-center">{keyObjectives.title}</h3>
                        <ul className="space-y-4">
                            {(keyObjectives.items || []).map((item, index) => (
      ...
      <section id="features" className="py-24 bg-[var(--color-background)]">
        <div className="container mx-auto px-6">
                    ...
                    {(features.items || []).map((feature, index) => {
                        return (
                            <Animated key={index} delay={100 + index * 100}>
                                <div className="group bg-[var(--color-card-background)] rounded-2xl shadow-lg p-8 flex flex-col transition-all duration-300 hover:shadow-xl hover:border-[var(--color-primary)]/50 border border-transparent h-full relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-4 mb-6 relative">
                                        {renderIcon(feature.icon, feature.title, "h-12 w-12 text-[var(--color-primary)]")}
            ...
            {activeTab === 'reports' && (
                 <div className="animate-tab-content-fade-in max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(reports.items || []).map((item, index) => (
                        <Animated key={index} delay={100 + index * 50}>
                             <div className="flex items-start text-base bg-white p-6 rounded-lg shadow-sm">
                                <ReportsIcon className="w-6 h-6 text-[var(--color-primary)] mr-4 mt-1 flex-shrink-0" />
                                <span className="text-[var(--color-text-secondary)]">{item}</span>
                            </div>
                        </Animated>
                    ))}
      ...
      {/* Summary Section */}
      <section className="py-24 bg-[var(--color-card-background)] border-t border-[var(--color-border)]">
         <div className="container mx-auto px-6 max-w-4xl text-center">
            <Animated>
                <h2 className="text-2xl md:text-3xl font-bold">جمع‌بندی</h2>
                <div className="w-24 h-1 bg-[var(--color-primary)] mx-auto mt-4 rounded-full"></div>
                <p className="mt-8 text-base md:text-lg text-[var(--color-text-secondary)] leading-relaxed">{summary}</p>
            </Animated>
        </div>
      </section>
